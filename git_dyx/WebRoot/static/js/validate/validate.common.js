 ...
 */
var validateForm = function(expr, callback) {
	// 表单对象
	var form = $(expr);
	// 表单校验器
	var vt = form.validate();
	// 校验表单
    vt.form();
	// 延迟获取校验结果
	deferredCall(function() {
		// 保存错误处理程序调用状态
		var key = "invokeInvalidHandler";
		// 设置错误处理程序调用状态为允许调用
		$.data(form[0], key, true);
		// 获取校验结果，并触发调用错误处理程序
		var result = vt.form();
		// 清除错误处理程序调用状态
		form.removeData(key);
...
var _invalidHandler = function(e, validator) {
	...
	if($.data(form, "invokeInvalidHandler")) {
		// 弹出错误信息
		window.top.dialogMessage(alertMessage, function(content) {
			// 关闭弹出窗口
